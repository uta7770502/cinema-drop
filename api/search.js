export default async function handler(req,res){
 ...
 try{
  ...
  if(memory){
   ...
   };let m=moods[mood]||moods.cozy;url=new URL("https://api.themoviedb.org/3/discover/movie");url.searchParams.set("language","ja-JP");url.searchParams.set("region","JP");url.searchParams.set("include_adult","false");url.searchParams.set("with_genres",m.genres.replaceAll("|",","));url.searchParams.set("sort_by",m.sort);url.searchParams.set("vote_count.gte",m.votes);url.searchParams.set("vote_average.gte",m.rating);url.searchParams.set("without_genres",m.without.replaceAll("|",","));url.searchParams.set("with_runtime.lte",m.runtime);url.searchParams.set("primary_release_date.lte",new Date().toISOString().slice(0,10));
  }else if(mode){
